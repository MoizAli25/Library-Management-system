...
def load_library():
    if os.path.exists(data_file):  # Fixed typo
        with open(data_file, 'r') as file:
            return json.load(file)
    return []  # Fixed: Return an empty list, not an empty array syntax
...
def remove_book(library):
    ...
    # Remove book by creating a new list without the book
    ...
    return updated_library  # Return the updated list
...
def display_statistics(library):
    ...
    unread_books = len([book for book in library if not book['read']])
    read_books = len([book for book in library if book['read']])

    percentage_read = (read_books / total_books) * 100 if total_books > 0 else 0

    print(f"Total number of books: {total_books}")
    print(f"Number of unread books: {unread_books}")
    print(f"Number of read books: {read_books}")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            library = remove_book(library)  # Update library after removing
        elif choice == '3':
